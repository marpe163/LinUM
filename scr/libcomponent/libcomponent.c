...
static int get_number_of_resistors_in_series(const float orig_resistance);
int e_resistance(float orig_resistance, float *res_array );

int main()
{
    float res[3];
    int count = e_resistance(1398.f, res);
    ...
    for(int i=0; i < e12Resistors.number_of_resistors; i++)
    {
        for(int j=i; j < e12Resistors.number_of_resistors; j++)
        {
            for(int k=j; k < e12Resistors.number_of_resistors; k++)
            {
                ...
                res_difference = orig_resistance - current_e_res;
                abs_error = res_difference < 0 ? res_difference * -1.f : res_difference;
...
static e12_series generate_e12_series(const float orig_resistance)
{
    int number_of_resistorsInSeries = get_number_of_resistors_in_series(orig_resistance);
    e12_series resistors = {.number_of_resistors = number_of_resistorsInSeries,
                            .resistor_values = calloc(number_of_resistorsInSeries, sizeof(float))};

    for(int resistor_idx=0; resistor_idx<number_of_resistorsInSeries - 1; resistor_idx++)
    {
...
static int get_number_of_resistors_in_series(const float orig_resistance)
{
    ...
    for(int resistor_idx = 0;;resistor_idx++)
    {
        currentVal = e12_series_coefficients[resistor_idx % 12] * powf(10.f, resistor_idx / 12);

        number_of_resistors++;

        if(currentVal > orig_resistance)
        {
             break;
        }
    }
    number_of_resistors++; //Include the zero-resistor
    return number_of_resistors;
}
